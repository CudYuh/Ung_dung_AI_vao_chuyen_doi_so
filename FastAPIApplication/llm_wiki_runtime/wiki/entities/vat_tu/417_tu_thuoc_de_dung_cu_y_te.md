# Tủ thuốc + để dụng cụ y tế

## Thông tin định giá

- ID database: 417.0
- Tên hàng hóa: Tủ thuốc + để dụng cụ y tế
- Loại hàng hóa: Chưa xác định
- Đơn vị tính: bộ
- Giá thẩm định: 3.000.000 VND
- Ngày thẩm định: 15/12/2025
- Chứng thư thẩm định: : 405/2025/294/CTTĐG-VNVIC
- Nguồn dữ liệu: …
- Người thẩm định: Bùi Thị Trà Giang

## Thông số kỹ thuật

Tủ thuốc + để dụng cụ y tế:
Inox SUS 201
- Kích thước: 800x400x1600
- Mô tả: Khung tủ, cánh tủ 25x25x0.6mm, đố tủ 25x50x0.8mm, tấm xung quanh, hồi tủ cánh tủ dày 0.4mm được gấp liền trên máy thủy lực, tủ được chia làm 2 khoang, khoang trên cánh kính có 2 đợt di động. Tủ dưới chia 2 ngăn nhỏ. Tủ có khóa phần trên và dưới.

## Vai trò trong LLM Wiki

Trang này là entity tri thức của vật tư **Tủ thuốc + để dụng cụ y tế**.

Entity này giúp hệ thống:

- Tra cứu lại thông tin định giá
- Giải thích nguồn dữ liệu
- Liên kết với concept nghiệp vụ
- Hỗ trợ AI Agent sử dụng lại tri thức trong các lần định giá sau

## Liên kết concept

- [[Giá tham chiếu]]
- [[Nguồn dữ liệu]]
- [[Quy tắc chọn giá]]
- [[Vật tư tương tự]]
- [[Second Brain]]

## Ghi chú đồng bộ

Trang được sinh tự động từ PostgreSQL thông qua LLM Wiki Framework.
