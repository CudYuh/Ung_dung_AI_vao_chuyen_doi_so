# Tháp khử mùi
 + Xuất xứ: Việt Nam
 + Vật liệu: Composite
 + Kích thước hình trụ tròn: D x L = 500 x 1200 mm
 + Vật liệu hấp phụ: than hoạt tính gáo dừa

## Thông tin định giá

- ID database: 691.0
- Tên hàng hóa: Tháp khử mùi
 + Xuất xứ: Việt Nam
 + Vật liệu: Composite
 + Kích thước hình trụ tròn: D x L = 500 x 1200 mm
 + Vật liệu hấp phụ: than hoạt tính gáo dừa
- Loại hàng hóa: Chưa xác định
- Đơn vị tính: Bộ
- Giá thẩm định: 27.273.000 VND
- Ngày thẩm định: 15/12/2025
- Chứng thư thẩm định: : 405/2025/294/CTTĐG-VNVIC
- Nguồn dữ liệu: …
- Người thẩm định: Bùi Thị Trà Giang

## Thông số kỹ thuật

Gia công theo bản vẽ chế tạo
Tháp khử mùi
 + Xuất xứ: Việt Nam
 + Vật liệu: Composite
 + Kích thước hình trụ tròn: D x L = 500 x 1200 mm
 + Vật liệu hấp phụ: than hoạt tính gáo dừa

## Vai trò trong LLM Wiki

Trang này là entity tri thức của vật tư **Tháp khử mùi
 + Xuất xứ: Việt Nam
 + Vật liệu: Composite
 + Kích thước hình trụ tròn: D x L = 500 x 1200 mm
 + Vật liệu hấp phụ: than hoạt tính gáo dừa**.

Entity này giúp hệ thống:

- Tra cứu lại thông tin định giá
- Giải thích nguồn dữ liệu
- Liên kết với concept nghiệp vụ
- Hỗ trợ AI Agent sử dụng lại tri thức trong các lần định giá sau

## Liên kết concept

- [[Giá tham chiếu]]
- [[Nguồn dữ liệu]]
- [[Quy tắc chọn giá]]
- [[Vật tư tương tự]]
- [[Second Brain]]

## Ghi chú đồng bộ

Trang được sinh tự động từ PostgreSQL thông qua LLM Wiki Framework.
