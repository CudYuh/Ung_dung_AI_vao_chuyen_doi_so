# Bục tượng bác

## Thông tin định giá

- ID database: 1845.0
- Tên hàng hóa: Bục tượng bác
- Loại hàng hóa: Chưa xác định
- Đơn vị tính: Cái
- Giá thẩm định: 3.409.560 VND
- Ngày thẩm định: 18/12/2025
- Chứng thư thẩm định: 405/2025/392/CTTĐG-VNVIC
- Nguồn dữ liệu: …
- Người thẩm định: Bùi Thị Trà Giang

## Thông số kỹ thuật

Bục tượng bác:
 - Kích thước:800x600x1160mm
 - Bục tượng bác được làm bằng gỗ công nghiệp sơn PU đồng màu với nội thất trong phòng
 - Kết hợp giấy vân đốm. Phần trên bục cách điệu đài sen trang trí

## Vai trò trong LLM Wiki

Trang này là entity tri thức của vật tư **Bục tượng bác**.

Entity này giúp hệ thống:

- Tra cứu lại thông tin định giá
- Giải thích nguồn dữ liệu
- Liên kết với concept nghiệp vụ
- Hỗ trợ AI Agent sử dụng lại tri thức trong các lần định giá sau

## Liên kết concept

- [[Giá tham chiếu]]
- [[Nguồn dữ liệu]]
- [[Quy tắc chọn giá]]
- [[Vật tư tương tự]]
- [[Second Brain]]

## Ghi chú đồng bộ

Trang được sinh tự động từ PostgreSQL thông qua LLM Wiki Framework.
