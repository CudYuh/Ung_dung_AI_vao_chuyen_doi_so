# Kệ thiên nhiên

## Thông tin định giá

- ID database: 1445.0
- Tên hàng hóa: Kệ thiên nhiên
- Loại hàng hóa: Chưa xác định
- Đơn vị tính: Cái
- Giá thẩm định: 1.770.000 VND
- Ngày thẩm định: 18/12/2025
- Chứng thư thẩm định: 405/2025/392/CTTĐG-VNVIC
- Nguồn dữ liệu: …
- Người thẩm định: Bùi Thị Trà Giang

## Thông số kỹ thuật

Kệ thiên nhiên
Kích thước: 600 x 300 x 1440 mm
Vật liệu: Khung sắt hộp 20 x 20 x 1,2 mm sơn tĩnh điện kết hợp các ngăn kệ gỗ cao su ghép thanh dày 15mm được thiết kế với nhiều ngăn kệ bố trí so le nhau tạo lên sự sinh động hơn trong các trang trí, có thể để 10-12 chậu cây với 7 tầng khác nhau.

## Vai trò trong LLM Wiki

Trang này là entity tri thức của vật tư **Kệ thiên nhiên**.

Entity này giúp hệ thống:

- Tra cứu lại thông tin định giá
- Giải thích nguồn dữ liệu
- Liên kết với concept nghiệp vụ
- Hỗ trợ AI Agent sử dụng lại tri thức trong các lần định giá sau

## Liên kết concept

- [[Giá tham chiếu]]
- [[Nguồn dữ liệu]]
- [[Quy tắc chọn giá]]
- [[Vật tư tương tự]]
- [[Second Brain]]

## Ghi chú đồng bộ

Trang được sinh tự động từ PostgreSQL thông qua LLM Wiki Framework.
