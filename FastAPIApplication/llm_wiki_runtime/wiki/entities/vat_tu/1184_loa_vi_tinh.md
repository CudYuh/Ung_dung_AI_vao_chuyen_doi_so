# Loa vi tính

## Thông tin định giá

- ID database: 1184.0
- Tên hàng hóa: Loa vi tính
- Loại hàng hóa: Chưa xác định
- Đơn vị tính: Bộ
- Giá thẩm định: 5.060.000 VND
- Ngày thẩm định: 16/12/2025
- Chứng thư thẩm định: 405/2025/252/CTTĐG-VNVIC
- Nguồn dữ liệu: …
- Người thẩm định: Bùi Thị Trà Giang

## Thông số kỹ thuật

Loa máy vi tính:                                         
- Công suất  38W, 
- Số lượng kênh : 2,1 kênh
- Kết nối không dây: Bluetooth 5.0
- Kết nối khác: Đầu AUX IN; Thẻ nhớ SD; USB
- Phím điều khiển: Chỉnh Bass; Phát/dừng chơi nhạc; Tăng/giảm âm lượng
- Gồm 1 loa trầm và 2 loa vệ tinh

## Vai trò trong LLM Wiki

Trang này là entity tri thức của vật tư **Loa vi tính**.

Entity này giúp hệ thống:

- Tra cứu lại thông tin định giá
- Giải thích nguồn dữ liệu
- Liên kết với concept nghiệp vụ
- Hỗ trợ AI Agent sử dụng lại tri thức trong các lần định giá sau

## Liên kết concept

- [[Giá tham chiếu]]
- [[Nguồn dữ liệu]]
- [[Quy tắc chọn giá]]
- [[Vật tư tương tự]]
- [[Second Brain]]

## Ghi chú đồng bộ

Trang được sinh tự động từ PostgreSQL thông qua LLM Wiki Framework.
